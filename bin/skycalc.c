...

{
	...
	printf("   f .. Fresno, California\n"); /* Installed in 2007 by F. Ringwald */
/*	printf("   s .. Shattuck Obs., Dartmouth College, Hanover NH\n"); */
	printf("   z .. SAAO, Sutherland\n");
	...
	if(obs_code[0] == 'x') {
		...
		*elev = 0.;  /* below surrouding horizon */
		...
		/* Installed in 2007 by F. Ringwald */
		strcpy(site_name, "Fresno, California");
		...
		*stdz = 8.;
		*elevsea = 105.;
		*elev = 0.;  /* neglect mountains in distance */
	}
...
/*      else if (obs_code[0] == 'c') {
		...
		*lat = get_coord();
		...
		scanf("%c",zabr);
		printf("Type daylight saving time option --- \n");
...
{
	/* finds jd's at which daylight saving time begins 
...

{
	...
	if(use_dst > 0) {
		oprntf("%s Daylight Saving Time assumed from 2 AM on\n",zone_name);
		...
		oprntf("%s Daylight Saving Time used before 2 AM \n",zone_name);
...
/* better reflect the updating staus in 'l' ... */
	...
	else oprntf("   USE_DST: %3d    -- Daylight saving used part of year.\n",      
			use_dst);
#if SYS_CLOCK_OK == 1
	if(update_on == 1) {
...

{
	...
	oprntf("\nDaylight saving time, if selected, is established using a\n");
	...
	if(ans > 1.0e-5) {
       ...
       if(enter_ut == 1) printf("Times & dates should be UT, ");
		...
		printf("Times and dates should be local, zone = %4.1f hr W\n");
	        if(use_dst != 0) printf("Daylight saving applies if current,\n");
